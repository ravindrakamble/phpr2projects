...
<div id="collapse4" class="body">
	...
	<br/>
	<form  method="post" action="<?php echo base_url()?>admin_c/outstation_package_commission">
	<?php
		$agent = array();
...
function fixed_save(id)
{
    ...
    jQuery('#blkyes').click(function() { 
        $.ajax({
			type:"POST",
			url: "<?php echo base_url();?>admin/comm_out_fixed_save/"+id+'/'+val,
...
function percentage_save(id)
{
    ...
    jQuery('#blkyes').click(function() { 
        $.ajax({
			type:"POST",
			url: "<?php echo base_url();?>admin/comm_out_percentage_save/"+id+'/'+val,
			success: function(data) {
				window.location.reload();
			}
		});
