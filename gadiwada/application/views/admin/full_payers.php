...
$time3 = '';
$deduction = '';
if(isset($full_payers_edit) && !empty($full_payers_edit))
{
	foreach($full_payers_edit as $info){
	$id = $info->id;
	$time1 = $info->time1;
	$time2 = $info->time2;
	$time3 = $info->time3;
...
?>
	<h3>For Full Payers</h3>
<form name="myform" method="post" action="<?php echo base_url()?>admin/cancellation/full/<?php echo $action?>">
...
<div id="collapse4" class="body">
	...
	<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped">
		...
		<tr>				
			...
			<td>
			<a href='<?php echo base_url().'admin_c/full_payers/'.$row->id ?>' class='ajax btn edit' alt='Edit'>
				<i class='icon-edit'></i>
				</a>
			&nbsp;&nbsp;
...
function payer_delete(id)
{
	...
	jQuery('#blkyes').click(function() { 
        $.ajax({
			type:"POST",
			url: "<?php echo base_url();?>admin/payer_delete/"+id+"/full",
